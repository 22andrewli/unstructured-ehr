...
export function searchEHRs(query: string): {
  ...
  if (terms.length === 0) {
    return {
      results: [],
      stats: { patientCount: 0, encounterCount: 0, avgEncountersPerPatient: 0 },
    };
  }
      ...
      if (isMatch) {
        ...
        if (highlights.length > 0) {
          matchedResults.push({
            ...
            matchedText: text,
            highlights,
            totalEncounters: patient.encounters,
          });
  ...
  return {
    ...
    stats: {
      ...
      avgEncountersPerPatient: patientCount > 0 
        ? Math.round((totalEncounters / patientCount) * 10) / 10 
        : 0,
    },
  };
}
